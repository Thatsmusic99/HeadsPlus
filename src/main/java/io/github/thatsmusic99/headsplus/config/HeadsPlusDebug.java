...
public class HeadsPlusDebug extends ConfigSettings {
    ...
    protected void load(boolean nullp) {
        ...
        getConfig().addDefault("event.death-event.logging", true);
    ...
    public void log(String name, DebugType type, String message) {
        if (getConfig().getBoolean(type.name().toLowerCase() + "." + name + ".logging")) {
            System.out.println("[HeadsPlus Debug] " + name + ": " + message);
        }
    }
